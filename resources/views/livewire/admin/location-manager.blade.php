<div class="min-h-screen p-6 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Manajemen Lokasi Aset</h2>
            <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">Atur data lokasi penempatan aset perusahaan.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-700/50">
                    <tr>
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nama Lokasi</th>
                        ...
                        <tr class="hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors duration-150 group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="text-sm font-bold text-slate-800 dark:text-white">{{ $location->name }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-slate-600 dark:text-slate-400 line-clamp-2">{{ $location->description ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                <button wire:click="edit({{ $location->id }})" class="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors p-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button wire:click="delete({{ $location->id }})"
                                    wire:confirm="Anda yakin ingin menghapus lokasi ini? Tindakan ini tidak dapat dibatalkan."
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
                                    ...
                                    <span class="text-base font-medium">Belum ada data lokasi yang tersedia.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div class="relative p-0 bg-white dark:bg-slate-800 w-full max-w-lg m-auto flex-col flex rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700">
                ...
                <div class="p-6">
                    <form wire:submit.prevent="store">
                        <div class="space-y-5">
                            <div>
                                <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nama Lokasi</label>
                                <input type="text" id="name" wire:model.defer="name" 
                                    ...
                                    placeholder="Contoh: Gudang Utama, Ruang Server...">
                            ...
                            <div>
                                ...
                                <textarea id="description" wire:model.defer="description" rows="4" 
                                    class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:focus:ring-blue-500/20 transition placeholder-slate-400" 
                                    placeholder="Detail mengenai lokasi ini..."></textarea>
