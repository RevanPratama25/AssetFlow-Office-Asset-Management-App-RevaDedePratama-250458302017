            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 transition-colors duration-300">
                ...
                @if (session()->has('success'))
                    <div class="bg-green-100 dark:bg-green-900 border-l-4 border-green-500 text-green-700 dark:text-green-200 p-4 mb-4" role="alert">
                        ...
                        <p><span class="block sm:inline">{{ session('success') }}</span></p>
                    </div>
                @endif
                    ...
                    <div class="fixed inset-0 z-50 overflow-auto bg-gray-500 bg-opacity-75 flex">
                        ...
                        <div class="relative p-8 bg-gray-500 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-90 transition-opacity w-full max-w-md m-auto flex-col flex rounded-lg">
                            ...
                            <form wire:submit.prevent="store">
                                ...
                                {{-- Modal panel --}}
                                <h2 class="mb-5 text-2xl leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">{{ $locationId ? 'Edit Lokasi' : 'Tambah Lokasi' }}</h2>
                                
                                <div class="mb-4">

                                    <label for="name" class="block text-gray-700 dark:text-white text-sm font-bold mb-2">Nama Lokasi:</label>
                                    
                                    <input type="text" id="name" wire:model.defer="name" class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md p-2" placeholder="Masukkan nama lokasi">
                            ...
                            @forelse ($locations as $location)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $location->name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-gray-300 line-clamp-2">{{ $location->description ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="edit({{ $location->id }})" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 mr-3 transition-colors duration-150">Edit</button>
                                ...
                                <tr>
                                    <td colspan="3" class="px-6 py-10 whitespace-nowrap text-center text-sm text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p>Belum ada data lokasi yang tersedia.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
